...
parser.add_argument("-s", "--status", help="Check status of request and download if complete (downloads to /tmp)")
parser.add_argument("-r", "--requestId", help="Request Id of mbox retrival request")
...
def requestMailboxDownload(url, token):
    """Sets up request to download mailbox data"""
    postData = formatMailboxDownloadRequestXml()
    print postData
    responseData = postToGoogle(publicKeyUploadHeader, postData, url, token)
    print responseData


def getMailboxExportStatus(url, requestId, authToken, email, epoch):
    """Print mailbox status and download mbox file if status is Complete"""
    status = getFromGoogle(requestId, url, authToken)
...
if args.auth:
    ...
    if os.path.exists(cacheFileLocation):
        ...
        SID, LSID, Auth = authToken.split("\n")
        SID = SID.split("=")[1]
        LSID = LSID.split("=")[1]
        Auth = Auth.split("=")[1]
        authToken = {"token": {"SID": SID, "LSID": LSID, "Auth": Auth}}
        with open(cacheFileLocation, 'w') as cacheFile:
            cacheFile.write(json.dumps(authToken))
...
if args.requestId:
    print requestMailboxDownload(mailboxExportUrl, authToken)
    pass

if args.status:
    # Check mailbox status download if status is Complete
    #getMailboxExportStatus(mailboxExportStatusUrl, "427823502", args.token, args.email, epoch)
    pass
